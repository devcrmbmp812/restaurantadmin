   ...
   lastIndex=findLength(value);
    ...
    angular.forEach(value, function(value, key) {
        ...
        if(value.orderStatus==status) {
            ...
            //    '<td>' + data.quantity + '</td>' +
            //    '<td>R$' + data.total + '</td>' +
            //    '<td>' + '<button type="button"  id="myBtn" onclick="orderDetail(' + ket + ')" style="background-color: green; color: white">Detalhes</button>' + '</td>' +
            //
            //    '</tr>';
...
function populateUiNew(obj){
    ...
    angular.forEach(orders, function(value, key) {
        ...
        var html = '<tr> ' +
            ...
            '<td>R$' + value.total + '</td>' +
            '<td>' + '<button type="button"  id="myBtn" onclick="orderDetail(' + value.ket + ')" style="background-color: green; color: white">Detalhes</button>' + '</td>' +
...
function orderDetail(key,email,name,phone,address,city,uKey,status,cellPhone,state,zip,username,cpf){
  ...
  angular.forEach(order.orderProducts, function(value, key) {
        productsHtml+='<tr><td style="width: 40%;">'+value.name+'<br> <span style="font-size: small"> '+value.description +' </span></td>'
        +'<td style="margin-left: 40%">'+value.quantity+'</td>'
        +'<td>R$'+(value.price*value.quantity).toFixed(2)+'</td></tr>';
      ...
      var html='<tr>   ' +
          ...
          '</tr>' +
          '<tr >' +
          '  <td><b> Celular</b></td><td>'+cellPhone+' </td> <td> <b>Cidade </b> </td> <td>'+ city +  '</td> </tr>'
          +
              '<tr >' +
          '  <td><b> Responsavel </b></td><td>'+username+' </td> <td> <b>Estado </b> </td> <td>'+ state +  '</td> </tr>' +

              '<tr >' +
          '  <td><b> DOC </b></td><td>'+cpf+' </td> <td> <b>CEP </b> </td> <td>'+ zip +  '</td> </tr>'

           +
          '<tr><td> <b>Data pedido </b> </td>' +
    ...
    if(status=="Pending") {
        ...
        document.getElementById('buttonsDiv').innerHTML = ' <div class="col-sm-4"> ' +
            '<button  data-dismiss="modal" type="button" onclick="acceptOrder('+ket+')" class="btn" style="background-color: #129621; color: white" >Aceitar</button> ' +
            '</div> ' +
            '<div class="col-sm-3"> ' +
            '<button type="button" class="btn" style="background-color: #d12923; color: white">Cancelar</button></div> ' +
            '<div class="col-sm-3"><button type="button" onclick="printDetails()" class="btn" style="background-color:#96999e; color: white">Imprimir</button> '+
            '</div>';
    }
    if(status=="Accepted"){
        document.getElementById('deliverBtnDiv').innerHTML ='<div class="col-sm-4 col-sm-offset-4"> ' +
            '<button data-dismiss="modal" type="button" onclick="deliverOrder('+ket+')"  class="btn btn-success" style="background-color: #129621; color: white; width: 100%" >Entregar agora</button> ' +
            '</div>'
